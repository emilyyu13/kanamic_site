...
<%-- メール送信  --%>
<%//入力値Validation
ResultOfValidate vRes = RequestValidationDl.getPackage(request).executeValidate();
...
//	mailBean.setHost("192.168.1.63");
	...
	mailBean.setSubject("【資料ダウンロード】お問合せフォームに登録がありました。");
	mailBean.setText(new RequestMailTextDl(request.getParameterMap()));

    try {
	    //　メール送信
	    SimpleSendMail.Send(mailBean);
	    //　送信が完了したら完了画面に切り替え
//	    RequestDispatcher dipatcher = request.getRequestDispatcher("complete_dl_dsp.jsp");
//	    dipatcher.include(request, response);
//    response.sendRedirect("complete_dl_dsp.jsp");
	    response.sendRedirect("https://www.kanamic.net/form/jIcrt8xXQsULgO1rOaa36ahJ56PlPme8h32A/complete_dl_dsp.jsp");
