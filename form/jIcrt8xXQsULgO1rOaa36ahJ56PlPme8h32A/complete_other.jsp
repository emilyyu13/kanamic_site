...
<%-- メール送信  --%>
<%//入力値Validation
ResultOfValidate vRes = RequestValidationOther.getPackage(request).executeValidate();
...
//	mailBean.setHost("192.168.1.63");
	...
	mailBean.setSubject("【その他問合せ】お問合せフォームに登録がありました。");
	mailBean.setText(new RequestMailTextOther(request.getParameterMap()));

    try {
	    //　メール送信
	    SimpleSendMail.Send(mailBean);
	    //　送信が完了したら完了画面に切り替え
//	    RequestDispatcher dipatcher = request.getRequestDispatcher("complete_other_dsp.jsp");
//	    dipatcher.include(request, response);
//	    response.sendRedirect("complete_other_dsp.jsp");
	    response.sendRedirect("https://www.kanamic.net/form/jIcrt8xXQsULgO1rOaa36ahJ56PlPme8h32A/input.jsp");
